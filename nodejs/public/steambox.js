/* ============================================================================
   ...
   - Strict Alignment with Master dashboard.js & master_final.txt
   - Dynamic Recipe Sync with Native SCADA & Live Memory
   - Trolly free string input support
   - Safe Read-only SCADA Memory Resolution & Multi-channel Tag Writer
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            if (item) {
                for (const vName of variants) {
                    if (item.FullName === vName || item.Name === vName || item.FullName === tagName || key === tagName) {
                        return item.ID !== undefined ? item.ID : key;
                    }
                }
            }
        }
    ...
    setInterval(() => {
        ...
        for (let id = 1; id <= 30; id++) {
            ...
            const elSuhuAwal = document.getElementById(`sb_${id}_suhu_awal`);
            const elTemp = document.getElementById(`sb_${id}_temp`);
            const elSuhuAkhir = document.getElementById(`sb_${id}_suhu_akhir`);

            if (elSuhuAwal) elSuhuAwal.innerText = suhuAwalFmt;
            if (elTemp) elTemp.innerText = tempFormatted;
            if (elSuhuAkhir) elSuhuAkhir.innerText = suhuAkhirFmt;
            ...
            const elRecNama = document.getElementById(`recipe_${id}_nama`);
            const elRecKode = document.getElementById(`recipe_${id}_kode`);
            const elRecVersi = document.getElementById(`recipe_${id}_versi`);
            const elRecTrolly = document.getElementById(`recipe_${id}_trolly`);
            const elRecBatch = document.getElementById(`recipe_${id}_batch`);
            const elRecWarna = document.getElementById(`recipe_${id}_warna`);
            const elRecQty = document.getElementById(`recipe_${id}_qty`);

            if (elRecNama) elRecNama.innerText = recNama;
            if (elRecKode) elRecKode.innerText = recKode;
            if (elRecVersi) elRecVersi.innerText = recVersi;
            if (elRecTrolly) elRecTrolly.innerText = recTrolly;
            if (elRecBatch) elRecBatch.innerText = recBatch;
            if (elRecWarna) elRecWarna.innerText = recWarna;
            if (elRecQty) elRecQty.innerText = recQty;
            ...
            const elJamMulai = document.getElementById(`sb_${id}_jam_mulai`);
            const elJamMasak = document.getElementById(`sb_${id}_jam_masak`);
            const elJamSelesai = document.getElementById(`sb_${id}_jam_selesai`);
            const elPemanasan = document.getElementById(`sb_${id}_pemanasan`);

            if (elJamMulai) elJamMulai.innerText = getTagValue(scadaMap, [`sb_${id}.tampil_jam_mulai`, `sb${id}.tampil_jam_mulai`], '--:--:--');
            if (elJamMasak) elJamMasak.innerText = getTagValue(scadaMap, [`sb_${id}.tampil_jam_masak`, `sb${id}.tampil_jam_masak`], '--:--:--');
            if (elJamSelesai) elJamSelesai.innerText = getTagValue(scadaMap, [`sb_${id}.tampil_jam_selesai`, `sb${id}.tampil_jam_selesai`], '--:--:--');
            if (elPemanasan) elPemanasan.innerText = getTagValue(scadaMap, [`sb_${id}.tampil_pemanasan`, `sb${id}.tampil_pemanasan`], '--:--:--');
            ...
            const elTargetMenit = document.getElementById(`sb_${id}_target_menit`);
            const elPerubahanWaktu = document.getElementById(`sb_${id}_perubahan_waktu`);
            if (elTargetMenit) elTargetMenit.innerText = `${targetMin} menit`;
            if (elPerubahanWaktu) elPerubahanWaktu.innerText = `${adjMin} menit`;
    ...
    window.switchRecipeTab = function(tabName) {
        ...
        if (tabName === 'master') {
            if (tabMasterBtn) tabMasterBtn.classList.add('active');
            if (tabCustomBtn) tabCustomBtn.classList.remove('active');
            if (recipeTabMaster) recipeTabMaster.style.display = 'block';
            if (recipeTabCustom) recipeTabCustom.style.display = 'none';
        } else {
            if (tabCustomBtn) tabCustomBtn.classList.add('active');
            if (tabMasterBtn) tabMasterBtn.classList.remove('active');
            if (recipeTabCustom) recipeTabCustom.style.display = 'block';
            if (recipeTabMaster) recipeTabMaster.style.display = 'none';
        }
    };
    ...
    window.submitCustomRecipe = function() {
        const id = targetUnitForRecipe;
        const kodeEl = document.getElementById('inp_kode');
        const namaEl = document.getElementById('inp_nama');
        const versiEl = document.getElementById('inp_versi');
        const warnaEl = document.getElementById('inp_warna');
        const qtyEl = document.getElementById('inp_qty');
        const trollyEl = document.getElementById('inp_trolly');
        const batchEl = document.getElementById('inp_batch');
        const durasiEl = document.getElementById('inp_durasi');
        const chkMasterEl = document.getElementById('chk_save_master');

        const kode = (kodeEl && kodeEl.value.trim()) || 'sbmk';
        const nama = (namaEl && namaEl.value.trim()) || 'custom';
        const versi = (versiEl && versiEl.value.trim()) || '1';
        const warna = (warnaEl && warnaEl.value.trim()) || 'kuning';
        const qty = (qtyEl && qtyEl.value.trim()) || '125';
        const trolly = (trollyEl && trollyEl.value.trim()) || 'rapat';
        const batch = (batchEl && batchEl.value.trim()) || '1';
        const durasi = (durasiEl && durasiEl.value.trim()) || '1';
        const shouldSaveMaster = chkMasterEl ? chkMasterEl.checked : false;
