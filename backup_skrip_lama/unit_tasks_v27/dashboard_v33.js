/* ============================================================================
   STEAMBOX 30 UNIT DASHBOARD JAVASCRIPT LOGIC - VERSION v33 (AUTHENTICATED CONN SYNC)
   100% Exact Physical ID Tag Resolution (Compiled SCADA Engine Spec)
   - Emits official socket.emit("conn", connId) for SCADA session handshake
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ------------------------------------------------------------------------
    // 1. GET NATIVE SCADA `variables` DICTIONARY FROM GLOBAL SCOPE
    // ------------------------------------------------------------------------
    function getScadaVarsMap() {
        if (!window.variables) window.variables = {};
        if (window.variables && typeof window.variables === 'object') {
    ...
    // Process incoming realtime tag dictionary or array
            ...
            for (const id in data) {
                ...
                if (varsMap[strId]) {
                    ...
                    if (typeof val === 'object' && val !== null) {
                        ...
                        varsMap[strId] = { ID: Number(id), Value: val };
                    }
                }
            }
        }
    ...
    // ------------------------------------------------------------------------
    // 2. REAL-TIME SOCKET.IO ENGINE WITH AUTHENTICATED HANDSHAKE
    // ------------------------------------------------------------------------
    let socket = null;
    if (typeof io === 'function') {
        try {
            ...
            socket.on('connect', () => {
                // Emitting official SCADA handshake "conn"
            ...
            // Listen to ALL Haiwell WebServer realtime variable push events
            socket.on('return all var to browser', (data) => {
                updateVarsFromPayload(data);
            });
            ...
            socket.on('varChange', (data) => {
                updateVarsFromPayload(data);
            });

            socket.on('var to browser', (data) => {
                updateVarsFromPayload(data);
            });

            // Request full tag refresh every 500ms to guarantee 100% sync
            setInterval(() => {
    ...
    // ============================================================================
    // UNIVERSAL SCADA VARIABLE RESOLVER
    // ============================================================================
    ...
    // STRICT PHYSICAL TAG ID RESOLVER & MULTI-FORMAT WRITE DISPATCHER
    function setTagValue(tagName, value) {
        ...
        if (tagId) {
            // 1. Native Haiwell `Variable.SetById(id, value)`
            try {
                ...
                } else if (parent && parent.Variable && typeof parent.Variable.SetById === 'function') {
                    parent.Variable.SetById(Number(tagId), boolVal);
                }
            } catch(e) {}

            // 2. Direct Socket.io Emit Dispatcher
            try {
                let sock = window.activeSocket || window.socket || (parent && parent.socket);
                if (sock && typeof sock.emit === 'function') {
                    sock.emit("SetById", Number(tagId), valStr, "WRITE");
                    sock.emit("SetById", Number(tagId), boolVal, "WRITE");
                    sock.emit("SetById", Number(tagId), rawVal, "WRITE");
                }
            } catch(e) {}
        }
    }
    ...
    // ------------------------------------------------------------------------
    // STRICT SCADA COMMAND HANDLER (EXACT COMPILED PHYSICAL TAG SPECIFICATION)
    // ------------------------------------------------------------------------
    window.handleCommand = function(unitId, commandType) {
        ...
        if (commandType === 'tugas_baru') {
            ...
            const modeVal = getTagValue(scadaMap, [`sb_${unitId}.mode_preheat`, `sb${unitId}.mode_preheat`], 0);
            ...
            const runStopVal = getTagValue(scadaMap, [`sb${unitId}.run_stop`, `sb_${unitId}.run_stop`], 1);
            ...
            // Physical Reset Trigger: $sb_X.reset (Unit 1: ID 1196)
