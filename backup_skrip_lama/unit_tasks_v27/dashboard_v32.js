/* ============================================================================
   STEAMBOX 30 UNIT DASHBOARD JAVASCRIPT LOGIC - VERSION v32
   100% Haiwell Native `Variable.SetById` Engine (Exact iframe Page 5 Method)
   Strict Rule #7 Compliance: Dedicated Version File v32
   ============================================================================ */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof io === 'function') {
        try {
            ...
            // Haiwell SCADA "return var to browser" updates global `variables` object
    ...
    // STRICT HAIWELL NATIVE `Variable.SetById` & MULTI-FORMAT SOCKET WRITE DISPATCHER
    function setTagValue(tagName, value) {
        ...
        if (tagId) {
            // 1. Official Haiwell Native `Variable.SetById` (Exact method used by lib.js)
            try {
                if (window.Variable && typeof window.Variable.SetById === 'function') {
                    ...
                    parent.Variable.SetById(Number(tagId), rawVal);
                }
            } catch(e) {}

            // 2. Direct Socket.io Emit Dispatcher (Haiwell WebServer Backend)
            try {
    ...
    // ------------------------------------------------------------------------
    // STRICT SCADA COMMAND HANDLER (STANDALONE WEBSERVER ONLY)
    // ------------------------------------------------------------------------
